...
/**
 * This little program loads a CR report, merges another CR report and saves the
 * merges report to a third file.
 */
public class ConsoleMerger implements ReportMerger.Loader, ReportMerger.AssignData {
  ...
  public void run() {
    ...
    try {
      ...
      CRWriter crw = new CRWriter(merged, ui, type, merged.getEncoding());
  ...
  public static void main(String[] args) {
    ...
    if (args.length != 4) {
      ...
      System.out
          .println("  result_report - the destination file. If the file exists, it'll be overwritten.");
...
class NullFileTypeChooser extends FileTypeChooser {
  ...
  public ZipEntry chooseZipEntry(ZipEntry[] entries) {
    for (ZipEntry entry : entries) {
      ...
      if (name.toLowerCase().endsWith(".cr"))
        return entry;
    }
    return null;
  }

...
public class CRParser extends AbstractReportParser implements RulesIO, GameDataIO {
   ...
   *
   * @param ui The UserInterface for the progress. Can be NULL. Then no operation
   *           is displayed.
   */
  ...
  /**
   * Creates a new parser. This new parser translates coordinates according to
   * newOrigin. All
   * coordinates which are read from the report are translated by newOrigin. That
   * is, if a
   * coordinate read and its level (the z coordinate) equals the new origins
   * level, its x and y
   * coordinates are decreased by origin.x and origin.y, respectively. That means,
   * that the reports
   * origin is transferred to newOrigin.
   *
   * @param translator The coordinates (relative to the origin of the report) of
   *                   the new origin.
   */
  ...
  /**
   * Returns the value of the configuration tag ("Konfiguration") as it has been
   * read from the CR.
   *
   * @return The configuration string or <code>null</code> if the tag hasn't been
   *         read (yet)
   */
  ...
  /**
   * Print an error message on the standard output channel. Does not produce
   * duplicate messages.
   *
   * @param context The context (usually a block) within the error has been found.
   * @param fetch   If this is true, read the next line and skip the line with the
   *                error. Otherwise
  ...
  /**
   * Read the MESSAGETYPES block. Note that message type stubs have already been
   * created by parsing
  ...
  /**
   * Read a MESSAGETYPE block. Note that message type stubs have already been
   * created by parsing the
   * messages themselves.
   *
   * @throws IOException if the scanner throws an IOException
   */
  ...
  /**
   * Handle a sequence of quoted strings, storing them as <kbd>String</kbd>
   * objects. String
  ...
  /**
   * Parse the SPRUECHE sub block of UNIT and add them as <kbd>Spell</kbd>
   * objects.
   *
  ...
  /**
   * Parse a KAMPFZAUBER sub block of UNIT and add it as <kbd>CombatSpell</kbd>
   * object.
   *
  ...
  /**
   * Parse message blocks as can be found in cr versions &ge; 41. This function
   * evaluates only two
   * special message attributes. These are the ";type" and ";rendered" attributes,
   * which are
   * directly accessible in the <kbd>Message</kbd> object as type and text. If
   * there is no MessageType
   * object for this type of message, a stub MessageType object is created and
   * added to world.
   *
    ...
    while (sc.isBlock && sc.argv[0].startsWith("MESSAGE ")) {
      ...
      while (!sc.eof && !sc.isBlock) {
        if ((sc.argc == 2) && sc.argv[1].equalsIgnoreCase("type")) {
          ...
          // [PATCH] FIX: Force storage of 'section' as a standard attribute
          // so it can be retrieved by CRWriter/ConsoleMerger later.
          msg.setAttribute("section", section);
        } else if (sc.argc == 2) {
  ...
  /**
   * Parse a battlespec block sequence. Currently this is a block of message
   * blocks.
   *
  ...
  /**
   * Parse a sequence of spell blocks. Do not confuse this with the spells block
   * of a unit!
   *
   * @throws IOException if the scanner throws an IOException
   */
  private void parseSpells() throws IOException {
    while (!sc.eof && sc.isBlock && sc.argv[0].startsWith("ZAUBER ")) {
      ...
      // not adding spell immediately is required here, please do not change this,
      // unless you really
  ...
  public synchronized Map<String, Object> readHeader(Reader in) throws java.io.IOException {
    ...
    /*
     * Now read as long as the file lasts, or until a new block is found, which will
     * terminate the
     * header.
     */
    sc.getNextToken();
  ...
  private void parseHeader() throws IOException {
    ...
    while (!sc.eof) {
      if ((sc.argc == 2) && sc.argv[1].equalsIgnoreCase("Spiel")) {
        ...
        /**
         * assuming we have already the gamename we can make an additional check Buck
         * Tracking wrong
         * base...
         */
  ...
  private void parseItemType(Rules rules) throws IOException {
    ...
    while (!sc.eof) {
      if ((sc.argc == 2) && sc.argv[1].equalsIgnoreCase("weight")) {
        ...
        final Skill useSkill = new Skill(rules.getSkillType(StringID.create(sc.argv[0]), true), 0, 1, 0, false);
        itemType.setUseSkill(useSkill);
        sc.getNextToken();
        // darcduck - 20.11.2007 added magic bag tag that indicates if an item can be
        // stored in the
        // magic bag
   ...
   *
   * @param in The reader that will read the file for us.
   * @return a Rules object, or <code>null</code>, if the file hasn't been a
   *         ruleset.
   * @throws IOException If an I/O error occurs
   */
  private synchronized Rules readRules(Reader in) throws IOException {
    ...
    /*
     * The desired header has been parsed. Continue parsing the subsequent rule
     * blocks until the
     * file ends.
     */
    parseRules(rules);
  ...
  private void parseCoordinateTransformation() throws IOException {
    final EntityID id = EntityID.createEntityID(Integer.parseInt(sc.argv[0].substring("COORDTRANS".length() + 1)),
        world.base);
    sc.getNextToken();
    ...
    while (!sc.eof) {
      if ((sc.argc == 2) && sc.argv[1].equalsIgnoreCase("translation")) {
        ...
        // first mirror the translation at the origin (of the correct layer!), then
        // translate, then
        // mirror again:
  ...
  /**
   * This is the new version, the old is called "ALLIERTE" Heuristic for end of
   * block detection:
  ...
  /**
   * NOT IMPLEMENTED YET This is the old "ALLIERTE" version. Heuristic for block
   * termination: -
   * Terminate on any other block This method isn't implemented yet. It skips the
   * entire "ALLIERTE"
   * block.
   */
  ...
  /**
   * This is the old "ADRESSEN" version. Heuristic for block termination: -
   * Terminate on any other
   * block This method isn't implemented yet. It skips the entire "ADRESSEN"
   * block.
   *
   * @throws IOException If an I/O error occurs
   */
  ...
  /**
   * Parse the FACTION block with all its subblocks. Heuristic for block
   * termination:<br />
  ...
  private Options parseOptions(Options options) throws IOException {
    ...
    while (!sc.eof) {
      if (sc.argc == 2) {
        // options.setActive(StringID.create(sc.argv[1]), Integer.parseInt(sc.argv[0])
        // != 0);
        sc.getNextToken();
      } else if (sc.isBlock) {
        break;
  ...
  /**
   * Accesses unit.persons which must be &gt; 0 and just adds new skills, existing
   * skills are not
   * deleted.
   *
   * @throws IOException if the scanner throws an IOException
  ...
  private void parseSkills(Unit unit) throws IOException {
    ...
    while (!sc.eof && (sc.argc == 2)) {
      ...
      final Skill skill = new Skill(world.getRules().getSkillType(StringID.create(sc.argv[1]), true), points,
  ...
  private void parseItems(Unit unit) throws IOException {
    ...
    while (!sc.eof && (sc.argc == 2)) {
      final Item item = new Item(world.getRules().getItemType(StringID.create(sc.argv[1]), true), Integer
          .parseInt(sc.argv[0]));
      if (unit != null) {
        unit.addItem(item);
      }
  ...
  private int parseUnit(Region region, int sortIndex, String blockName) throws IOException {
    boolean oldUnit;
    final Unit unit = getAddUnit(UnitID
        .createUnitID(sc.argv[0].substring(blockName.length() + 1), 10, world.base),
        oldUnit = blockName.equals("ALTEINHEIT"));
    ...
    while (!sc.eof) {
      if ((sc.argc == 2) && sc.argv[1].equalsIgnoreCase("Name")) {
        ...
        final Item item = new Item(world.getRules().getItemType(EresseaConstants.I_USILVER, true), money);
        unit.addItem(item);
        sc.getNextToken();
      } else if ((sc.argc == 2) && sc.argv[1].equalsIgnoreCase("Burg")) {
        Integer.parseInt(sc.argv[0]);

        final Building b = getAddBuilding(EntityID.createEntityID(Integer.parseInt(sc.argv[0]), world.base));
        ...
        /*
         * currently, verkleidung was announced but it seems that anderepartei is used.
         * Please
        ...
        /*
         * currently (2004-02) the cr is inconsistent with nr. There may be a situation
         * where the
    ...
    /*
     * a missing combat status can have two meanings: 1. this is a unit we know
     * everything about and
     * the combat status is AGGRESSIVE 2. this is a unit we just see but does not
     * belong to us so we
  ...
  /*
   * Syntax: value;item Example: 24;Balsam < 0: offered in this region > 0:
  ...
  private void parseShip(Region region, int sortIndex) throws IOException {
    ...
    while (!sc.eof) {
      if ((sc.argc == 2) && sc.argv[1].equalsIgnoreCase("Name")) {
        ...
        // if ((ship.getOwnerUnit() != null) && (ship.getOwnerUnit().getFaction() ==
        // null)) {
        // final Faction f =
        // world.getFaction(EntityID.createEntityID(Integer.parseInt(sc.argv[0]),
        // world.base));
        ...
        if (version == 65) {
          // in E3 this tag was overloaded with the max number of persons (see Eressea bug
          // #1645)!
  ...
  private void parseBuilding(Region region, int sortIndex) throws IOException {
    ...
    while (!sc.eof) {
      if ((sc.argc == 2) && sc.argv[1].equalsIgnoreCase("Name")) {
        ...
        // this info is largely redundant (and destroys the faction's unit order)
        // if ((bld.getOwnerUnit() != null) && (bld.getOwnerUnit().getFaction() ==
        // null)) {
        // final Faction f = world.getFaction(EntityID.createEntityID(sc.argv[0], 10,
        // world.base));
    ...
    while (!sc.eof) {
      if ((sc.argc == 2) && sc.argv[1].equalsIgnoreCase("richtung")) {
        ...
        try {
          ...
          final String dirNames[] = { "Nordwesten", "Nordosten", "Osten", "S?dosten", "S?dwesten", "Westen" };
  ...
  /*
   * Parse everything within one region. Heuristic for block termination: -
   * Terminate on another
   * REGION or SPEZIALREGION block (without warning) - Terminate on any other
   * unknown block (with
   * warning)
   */
  private void parseRegion(int sortIndex) throws IOException {
    ...
    while (!sc.eof) {
      if ((sc.argc == 2) && sc.argv[1].equalsIgnoreCase("Name")) {
        ...
        final Faction f = world.getFaction(EntityID.createEntityID(Integer.parseInt(sc.argv[0]), world.base));
        ...
        /*
         * } else if(sc.argc == 1 && sc.argv[0].equals("REGIONSKOMMENTAR")) {
         * region.comments =
  ...
  /**
   * This function parses the informations found in Reader in and creates a
   * corresponding GameData
   * object tree.
   *
   * @param in   Reader to cr file
   * @param data GameData to be filled with informations of given cr file This
   *             function is
   *             synchronized.
   * @throws IOException If an I/O error occurs
   * @see magellan.library.io.GameDataIO#read(java.io.Reader,
  ...
  private void setOwner(GameData newData) {
    if (newData.getOwnerFaction() == null) {
      // in standard reports, the first faction of the report should always be the
      // owner faction
  ...
  // (stm) the following is an experiment to simplify the parser using a tag map
  // and Java Beans. It
  // didn't bring the performance boost I had hoped for...

  /**
   * A state object that is maintained by the parser. Individual handlers should
   * implement their own
   * state.
   *
   * @author stm
  ...
  /**
   * A class that handles individual tags or blocks as they are read by the
   * parser.
   *
   * @author stm
   */
  ...
  public static class BlockParser {
    ...
    /**
     * Adds a handler that is called if the specified tag (i.e., a line like
     * "value;tagName" is
     * encountered.
     *
    ...
    /**
     * Adds a handler that does nothing (but doesn't cause an error) for the
     * specified tag.
     *
    ...
    /**
     * Parses the block by calling {@link Scanner#getNextToken()} and calling
  ...
  private int parseUnitFast(Region region, int sortIndex, String blockName) throws IOException {
    ...
    final Unit newUnit = getAddUnit(UnitID
        .createUnitID(sc.argv[0].substring(blockName.length() + 1), 10, world.base),
        oldUnit = blockName.equals("ALTEINHEIT"));
     ...
     * sc\.argv\[1\]\.equalsIgnoreCase\(("[^"]*")\)\)
     * \{(\R[^#]*)sc\.getNextToken\(\);\R \}#else
     * parser.addTagHandler(\1, new TagHandler() { public void handle(Unit
     * unit){\2}}); if
     * \(\(sc\.isBlock\) && sc\.argv\[0\]\.equals\(("[^"]*")\)\) \{(\R[^#]*)\R
     * \}#else
     * parser.addBlockHandler(\1, new TagHandler() { public void handle(Unit unit)
     * throws
     * IOException {\2}}); <code>
     */
    if (unitParser == null) {
      ...
      unitParser.addTagHandler("Partei", new UnitTagHandler(unitParser) {
        @Override
        public void handle(Unit unit) {
          parseState.factionID = (EntityID.createEntityID(Integer.parseInt(sc.argv[0]), world.base));
        }
      });
  ...
  private void parseRegionFast(int sortIndex) throws IOException {
    ...
    if (regionParser == null) {
      ...
      regionParser.addBlockHandler("ALTEINHEIT", new RegionTagHandler(regionParser) {
        @Override
        public void handle(Region region) throws IOException {
          parseState.unitSortIndex = parseUnitFast(region, ++parseState.unitSortIndex, "ALTEINHEIT");
        }
      });
